...
void callback(const std_msgs::Float64::ConstPtr& msg)
{
    if (msg->data > UPPER_LIMIT || msg->data < LOWER_LIMIT)
    {
        ...
        ROS_INFO("The value %f sent to the beta motor is out of the valid range of %f to %f.", msg->data, UPPER_LIMIT, LOWER_LIMIT);
...
{
    ros::init(argc, argv, "betaMotor");

    // Incoming messages
    ros::NodeHandle nh;
    ros::Subscriber sub = nh.subscribe("betaMotor", 1000, callback);

    // Outgoing messages
    //feedback  = nh.advertise<std_msgs::Float64>("betaFeedback", 1000);
    simulator = nh.advertise<std_msgs::Float64>("andreas_arm/beta_position_controller/command",   1000);
